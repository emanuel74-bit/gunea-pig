...
type SemanticPolicy = {
  ...
  executablePathSignals: string[];
  contextAllowanceKeys: string[];
};
...
function loadSemanticPolicy(): SemanticPolicy {
...
function walk(node: any, visitor: (value: any, pathParts: string[], parent?: any) => void, pathParts: string[] = [], parent?: any): void {
  visitor(node, pathParts, parent);
  if (Array.isArray(node)) node.forEach((item, index) => walk(item, visitor, [...pathParts, String(index)], node));
  else if (node && typeof node === "object") {
    for (const [key, value] of Object.entries(node)) walk(value, visitor, [...pathParts, key], node);
  }
}
...
function pathHasSignal(pathParts: string[], signals: string[]): boolean {
...
function isAllowedContext(pathParts: string[]): boolean {
  return pathHasSignal(pathParts, semanticPolicy.contextAllowanceKeys);
}

function isExecutablePath(pathParts: string[]): boolean {
  return pathHasSignal(pathParts, semanticPolicy.executablePathSignals);
}

function hasStructuredConditionContext(parent: any): boolean {
...
function checkDecisionCompleteness(file: string, doc: JsonMap): void {
