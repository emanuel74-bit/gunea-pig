...
import {
  collectRuntimeArtifactDeclarations,
  conventionYamlFiles,
  extractRawAiArtifactPathsFromText,
  finish,
  globMatches,
  isGlobPath,
  issue,
...
const declarationsByPath = new Map<string, any[]>();
...
for (const file of conventionYamlFiles(root)) {
  ...
  for (const raw of extractRawAiArtifactPathsFromText(readText(file))) {
    ...
    if (raw !== normalized) normalizationDrift.push({ file: relative, raw_path: raw, normalized_path: normalized });
  }
}
...
const artifactEntries: Record<string, any> = {};
...
const undeclaredReferences = references.filter(ref => {
  if (exactDeclarations.some(d => d.path === ref.normalized_path)) return false;
  if (globDeclarations.some(d => globMatches(d.path, ref.normalized_path))) return false;
  return true;
});

const duplicatePathConflicts: any[] = [];
for (const [artifactPath, entries] of declarationsByPath.entries()) {
  const ids = [...new Set(entries.map(e => e.artifact_id))];
  if (ids.length > 1) duplicatePathConflicts.push({ path: artifactPath, artifact_ids: ids, declared_by: entries.map(e => e.declared_by) });
}

const duplicateIdConflicts: any[] = [];
for (const [artifactId, entries] of declarationsById.entries()) {
  const paths = [...new Set(entries.map(e => e.path))];
  if (paths.length > 1) duplicateIdConflicts.push({ artifact_id: artifactId, paths, declared_by: entries.map(e => e.declared_by) });
}
