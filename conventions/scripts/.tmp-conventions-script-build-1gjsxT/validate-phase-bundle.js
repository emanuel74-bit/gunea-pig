...
import { asArray, getArg, issue, normalizeRelPath, readExecutorRoutes, readYamlFile, resolveConventionsRoot, finish, } from "./lib/common.js";
...
function hasForbiddenDumpKey(node) {
    if (Array.isArray(node))
        return node.some(hasForbiddenDumpKey);
    if (!node || typeof node !== "object")
        return false;
    for (const key of Object.keys(node)) {
        if (["all_conventions", "full_convention_package", "full_topology", "all_topologies"].includes(key))
            return true;
        if (hasForbiddenDumpKey(node[key]))
            return true;
    }
    return false;
}
function requireArray(bundle, field) {
