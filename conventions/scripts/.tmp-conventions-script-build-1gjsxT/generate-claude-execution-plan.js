...
import { finish, readExecutorRoutes, readYamlFile, resolveConventionsRoot, writeYamlFile } from "./lib/common.js";
...
function extractStepRoutes(sectionName) {
    ...
    for (const step of steps) {
        ...
        if (Array.isArray(declared)) {
            for (const route of declared)
                if (typeof route === "string" && !found.includes(route))
                    found.push(route);
        }
    }
    return found;
