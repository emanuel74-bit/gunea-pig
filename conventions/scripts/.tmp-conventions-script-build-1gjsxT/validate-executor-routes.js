...
import { collectLocalRequiredRoutes, finish, issue, readExecutorRoutes, resolveConventionsRoot, routeIdLooksValid, } from "./lib/common.js";
...
for (const [routeId, route] of routes.entries()) {
    ...
    if ("blocking" in route)
        issues.push(issue("error", "FORBIDDEN_BLOCKING_FIELD", `Route ${routeId} uses forbidden blocking field`));
    if ("failure_policy" in route)
        issues.push(issue("error", "FORBIDDEN_FAILURE_POLICY", `Route ${routeId} uses forbidden failure_policy field`));
