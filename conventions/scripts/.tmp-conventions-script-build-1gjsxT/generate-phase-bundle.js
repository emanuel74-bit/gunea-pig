...
import { asArray, getArg, readExecutorRoutes, readYamlFile, resolveConventionsRoot, writeYamlFile, issue, finish, normalizeRelPath, } from "./lib/common.js";
...
function readConvention(relPath) {
    return readYamlFile(path.join(root, relPath));
}
function sectionArray(file, key) {
...
for (const routeId of requiredExecutorRoutes) {
    ...
    if (route)
        routeContracts[routeId] = pickRoute(route);
}
