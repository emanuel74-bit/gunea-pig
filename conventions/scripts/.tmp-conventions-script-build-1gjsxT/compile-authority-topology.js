...
import { asArray, conventionYamlFiles, finish, issue, readYamlFile, rel, resolveConventionsRoot, writeYamlFile, } from "./lib/common.js";
...
for (const file of conventionYamlFiles(root)) {
    ...
    if (["PACKAGE_MANIFEST.yaml", "VALIDATION_REPORT.yaml"].includes(relative))
        continue;
    ...
    for (const key of relationshipKeys) {
        for (const concept of asArray(ownership[key])) {
            ...
            if (key === "owns")
                concepts[concept].owners.push(relative);
        }
    }
}
