...
const reportPath = path.join(
    ...
    ".ai",
    "reports",
    "script-verification-report.yaml",
);

const compiledScriptRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "conventions-script-build-"),
);
const tsc = childProcess.spawnSync(
...
if (tsc.status !== 0) {
    issues.push(
        issue(
            "critical",
            "SCRIPT_BUILD_FAILED",
            "TypeScript script build failed before verification",
            undefined,
            { stdout: tsc.stdout, stderr: tsc.stderr },
        ),
    );
}
...
if (fs.existsSync(sourceNodeModules) && !fs.existsSync(buildNodeModules)) {
    ...
    fs.symlinkSync(sourceNodeModules, buildNodeModules, linkType);
}
fs.writeFileSync(
    path.join(compiledScriptRoot, "package.json"),
...
function copyDirectory(source: string, target: string): void {
    fs.cpSync(source, target, {
        ...
        filter: (copiedPath) => {
            ...
            if (base.startsWith(".tmp-conventions-script-build-")) return false;
...
function runScript(
    ...
    const child = childProcess.spawnSync(
        process.execPath,
        [scriptAbs, "--root", fixtureRoot, ...args],
        {
            cwd: path.join(root, "scripts"),
            encoding: "utf8",
            maxBuffer: 1024 * 1024 * 4,
...
function makeFixture(name: string): string {
    const base = fs.mkdtempSync(
        path.join(os.tmpdir(), `conventions-script-verification-${name}-`),
    );
...
function routeAllowedWritePatterns(
    routes: Map<string, JsonMap>,
    routeId: string,
): string[] {
...
function writtenFilesAreAllowed(
    writtenFiles: string[],
    allowedPatterns: string[],
): string[] {
    return writtenFiles.filter(
        (file) =>
            file.startsWith(".ai/") &&
...
function assertScriptResultShape(test: TestCase, run: ScriptRun): void {
    ...
    if (!result) {
        issues.push(
            issue(
                "critical",
                "SCRIPT_RESULT_NOT_PARSEABLE",
                `Script ${test.script} did not print parseable YAML/JSON result`,
                undefined,
                { test: test.name, stdout: run.stdout, stderr: run.stderr },
            ),
        );
        return;
    }
    for (const field of ["script_id", "status", "errors", "warnings", "info"]) {
        if (!(field in result))
            issues.push(
                issue(
                    "error",
                    "SCRIPT_RESULT_FIELD_MISSING",
                    `Script result missing field ${field}`,
                    undefined,
                    { test: test.name, script: test.script },
                ),
            );
    }
    if (
        ...
        issues.push(
            issue(
                "error",
                "SCRIPT_RESULT_LIST_FIELD_INVALID",
                "Script result errors/warnings/info must be arrays",
                undefined,
                { test: test.name, script: test.script },
            ),
        );
    ...
    if (result.status !== test.expectedStatus) {
        issues.push(
            issue(
                "error",
                "SCRIPT_RESULT_STATUS_UNEXPECTED",
                `Script ${test.script} status ${result.status} did not match expected ${test.expectedStatus}`,
                undefined,
                { test: test.name },
            ),
        );
...
function mutateExecutorRoute(
    fixtureRoot: string,
    mutator: (routes: JsonMap) => void,
): void {
    mutateYaml(
        path.join(fixtureRoot, "executors", "conventions.executor-routes.yaml"),
        (doc) => mutator(doc.sections.executor_routes),
    );
}

function writePhaseBundleFixture(
    fixtureRoot: string,
    mutator?: (bundle: JsonMap) => void,
    writeManifest = true,
): void {
    const bundlePath = path.join(
        ...
        "phases",
        "verify",
        "mission_profile_selection.bundle.yaml",
    );
    const manifestPath = path.join(
        ...
        "phases",
        "verify",
        "mission_profile_selection.manifest.yaml",
    );
    const generated = runScript(
        fixtureRoot,
        "scripts/generate-phase-bundle.ts",
        ["--mission-id", "verify", "--phase-id", "mission_profile_selection"],
    );
    if (generated.exitCode !== 0)
        throw new Error(
            `Could not generate base phase bundle for fixture: ${generated.stderr}`,
        );
...
function testCases(): TestCase[] {
    return [
        ...
        },
        {
            ...
            category: "invalid_state",
            setup: (r) =>
                addFixtureConvention(
                    r,
                    "phases/conventions.unknown-route-fixture.yaml",
                    validConvention(
                        "phases/conventions.unknown-route-fixture.yaml",
                        "unknown_route_fixture",
                        {
                            sections: {
        ...
        },
        {
            ...
            setup: (r) =>
                addFixtureConvention(
                    r,
                    "phases/conventions.direct-script-fixture.yaml",
                    validConvention(
                        "phases/conventions.direct-script-fixture.yaml",
                        "direct_script_fixture",
                        {
                            sections: {
                                required_executor_routes: {
                                    before_phase_start: [
        ...
        },
        {
            ...
            setup: (r) =>
                addFixtureConvention(
                    r,
                    "core/conventions.duplicate-owner-fixture.yaml",
                    validConvention(
                        "core/conventions.duplicate-owner-fixture.yaml",
        ...
        },
        {
            ...
            setup: (r) =>
                addFixtureConvention(
                    r,
                    "core/conventions.unknown-ownership-key-fixture.yaml",
                    {
                        version: "1.0",
                        file: {
        ...
        },
        {
            ...
            setup: (r) =>
                addFixtureConvention(
                    r,
                    "core/conventions.malformed-ownership-fixture.yaml",
                    {
                        version: "1.0",
                        file: {
        ...
        },
        {
            ...
            setup: (r) =>
                addFixtureConvention(
                    r,
                    "core/conventions.missing-dependency-fixture.yaml",
                    validConvention(
                        "core/conventions.missing-dependency-fixture.yaml",
        ...
        },
        {
            ...
            setup: (r) =>
                addFixtureConvention(
                    r,
                    "core/conventions.stale-reference-fixture.yaml",
                    validConvention(
                        "core/conventions.stale-reference-fixture.yaml",
                        "stale_reference_fixture",
                        {
                            purpose:
                                "mentions core/conventions.authority-registry.yaml as stale text",
                        },
                    ),
                ),
            expectedExit: 1,
        ...
        },
        {
            ...
            setup: (r) =>
                addFixtureConvention(
                    r,
                    "phases/conventions.unknown-route-reference-fixture.yaml",
                    validConvention(
                        "phases/conventions.unknown-route-reference-fixture.yaml",
                        "unknown_route_reference_fixture",
                        {
                            sections: {
                                required_executor_routes: {
                                    before_phase_start: [
            ...
            setup: (r) =>
                fs.writeFileSync(
                    path.join(
                        r,
                        "core",
                        "conventions.malformed-yaml-fixture.yaml",
                    ),
            ...
            setup: (r) =>
                mutateYaml(
                    path.join(
                        r,
                        "reports",
                        "conventions.runtime-artifacts.yaml",
                    ),
                    (doc) => {
            ...
            preRun: (r) => {
                ...
                mutateYaml(
                    path.join(
                        ...
                        ".ai",
                        "topologies",
                        "runtime-artifact-topology.yaml",
                    ),
                    (doc) => {
        ...
        },
        {
            ...
            preRun: (r) =>
                writePhaseBundleFixture(r, (bundle) => {
                    bundle.required_executor_routes.push(
                        "unknown_route_inside_bundle",
                    );
                }),
            expectedExit: 1,
            expectedStatus: "fail",
        ...
        },
        {
            ...
            setup: (r) =>
                addFixtureConvention(
                    r,
                    "phases/conventions.semantic-placeholder-fixture.yaml",
                    validConvention(
                        "phases/conventions.semantic-placeholder-fixture.yaml",
                        "semantic_placeholder_fixture",
                        { sections: { executable_rule: "TODO" } },
                    ),
                ),
            expectedExit: 1,
            expectedStatus: "fail",
        ...
        },
        {
            ...
            setup: (r) =>
                addFixtureConvention(
                    r,
                    "phases/conventions.semantic-vague-fixture.yaml",
                    validConvention(
                        "phases/conventions.semantic-vague-fixture.yaml",
                        "semantic_vague_fixture",
                        {
                            sections: {
                                executable_rule:
                                    "someone should run validation",
                            },
                        },
                    ),
                ),
            ...
            setup: (r) => {
                mutateYaml(
                    path.join(
                        r,
                        "core",
                        "conventions.semantic-completeness.yaml",
                    ),
                    (doc) => {
                        doc.sections.vague_content_policy.vague_execution_phrases.push(
                            "magic vague action",
                        );
                    },
                );
                addFixtureConvention(
                    r,
                    "phases/conventions.semantic-dynamic-policy-fixture.yaml",
                    validConvention(
                        "phases/conventions.semantic-dynamic-policy-fixture.yaml",
                        "semantic_dynamic_policy_fixture",
                        { sections: { executable_rule: "magic vague action" } },
                    ),
                );
            },
            expectedExit: 1,
        ...
        },
        {
            ...
            setup: (r) =>
                addFixtureConvention(
                    r,
                    "phases/conventions.semantic-decision-fixture.yaml",
                    validConvention(
                        "phases/conventions.semantic-decision-fixture.yaml",
                        "semantic_decision_fixture",
                        {
                            sections: {
                                routing_decision: {
        ...
        },
        {
            ...
            setup: (r) =>
                writeYamlFile(
                    path.join(r, ".ai", "phase", "weak-output.yaml"),
                    {
        ...
        },
        {
            ...
            setup: (r) =>
                writeYamlFile(
                    path.join(r, ".ai", "phase", "phase-output.yaml"),
                    { artifact: "phase_output" },
                ),
            expectedExit: 0,
            expectedStatus: "pass",
        ...
        },
        {
            ...
            setup: (r) =>
                writeYamlFile(
                    path.join(r, ".ai", "gates", "manual.gate-result.yaml"),
                    {
        ...
        },
        {
            ...
            setup: (r) =>
                writeYamlFile(
                    path.join(r, ".ai", "phase", "source-output.yaml"),
                    { artifact: "phase_output", status: "complete" },
                ),
            expectedExit: 0,
            expectedStatus: "pass",
        ...
        },
        {
            ...
            setup: (r) =>
                writeYamlFile(path.join(r, ".ai", "revisions", "manual.yaml"), {
        ...
        },
        {
            ...
            setup: (r) => {
                ...
                const agentTaxonomy = readYamlFile(
                    path.join(r, "agents", "conventions.agent-taxonomy.yaml"),
                );
                const agentIds = Object.keys(
                    (agentTaxonomy.sections?.agent_definitions ??
                        {}) as JsonMap,
                );
                ...
                fs.writeFileSync(
                    ...
                    coreRoutes
                        .map(
                            (route) =>
                                `npm --prefix conventions/scripts run executor -- --route ${route}`,
                        )
                        .join("\n"),
                    "utf8",
                );
                for (const skill of skills) {
                    fs.mkdirSync(
                        path.join(projectRoot, ".claude", "skills", skill),
                        { recursive: true },
                    );
                    fs.writeFileSync(
                        path.join(
...
for (const test of tests) {
    const fixtureRoot = makeFixture(
        test.name.replace(/[^a-zA-Z0-9]+/g, "-").toLowerCase(),
    );
    try {
        ...
        const disallowedWrites = writtenFilesAreAllowed(
            run.writtenFiles,
            allowedPatterns,
        );
        ...
        if (!routes.has(test.routeId)) {
            issues.push(
                issue(
                    "critical",
                    "SCRIPT_TEST_ROUTE_UNKNOWN",
                    `Test route is not declared: ${test.routeId}`,
                    undefined,
                    testRecord,
                ),
            );
        ...
        if (run.exitCode !== test.expectedExit) {
            issues.push(
                issue(
                    "error",
                    "SCRIPT_EXIT_CODE_UNEXPECTED",
                    `Script ${test.script} exited ${run.exitCode}; expected ${test.expectedExit}`,
                    undefined,
                    { ...testRecord, stderr: run.stderr, stdout: run.stdout },
                ),
            );
        }
        ...
        for (const expectedCode of test.expectedErrorCodes ?? []) {
            if (!codes.includes(expectedCode)) {
                issues.push(
                    issue(
                        "error",
                        "SCRIPT_EXPECTED_ERROR_CODE_MISSING",
                        `Script ${test.script} did not emit expected error code ${expectedCode}`,
                        undefined,
                        testRecord,
                    ),
                );
        ...
        for (const expectedOutput of test.expectedOutputs ?? []) {
            if (!fs.existsSync(path.join(fixtureRoot, expectedOutput))) {
                issues.push(
                    issue(
                        "error",
                        "SCRIPT_EXPECTED_OUTPUT_MISSING",
                        `Script ${test.script} did not write expected output ${expectedOutput}`,
                        undefined,
                        testRecord,
                    ),
                );
        ...
        if (disallowedWrites.length) {
            issues.push(
                issue(
                    "error",
                    "SCRIPT_WROTE_OUTSIDE_ALLOWED_PATHS",
                    `Script ${test.script} wrote outside allowed paths`,
                    undefined,
                    { ...testRecord, allowedPatterns },
                ),
            );
        }
    } finally {
...
const untestedRoutes = [...routes.keys()]
    .filter(
        (routeId) =>
            routeId !== "validate_script_execution" &&
...
for (const routeId of untestedRoutes) {
    issues.push(
        issue(
            "error",
            "SCRIPT_ROUTE_UNTESTED",
            `Executor route has no script verification test: ${routeId}`,
        ),
    );
}
...
writeYamlFile(reportPath, {
    ...
    required_route_coverage:
        "every executor route except validate_script_execution must have declared coverage; selected representative cases are run live",
    ...
    deferred_live_cases: declaredTests
        ...
        .map((test) => test.name)
        .sort(),
    failed_test_count: issues.filter(
...
const finalErrors = issues.filter(
    (entry) => entry.severity === "error" || entry.severity === "critical",
);
...
fs.writeFileSync(
    path.join(root, ".ai", "validation", "verify-scripts.result.yaml"),
