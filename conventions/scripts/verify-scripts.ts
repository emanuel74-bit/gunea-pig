...
function testCases(): TestCase[] {
  return [
    ...
    { name: "safe structure change analysis observes deletion risk", routeId: "analyze_safe_structure_change", script: "scripts/analyze-safe-structure-change.ts", category: "operation_correctness", args: ["--change-kind", "deletion", "--candidate-file", "conventions/scripts/validate-executor-routes.ts"], preRun: r => { const collected = runScript(r, "scripts/collect-source-artifacts.ts"); if (collected.exitCode !== 0) throw new Error(`Could not collect source artifacts fixture: ${collected.stderr}`); }, expectedExit: 0, expectedStatus: "pass", expectedWarningCodes: ["ROUTE_BACKED_OR_ROUTE_DECLARING_CANDIDATE", "INBOUND_DEPENDENCIES_PRESENT", "BEHAVIOR_PRESERVATION_EVIDENCE_MISSING", "CONSUMER_UPDATE_PLAN_MISSING"], expectedOutputs: [".ai/source-artifacts/safe-structure-change-analysis.yaml", ".ai/reports/safe-structure-change-report.yaml"], assert: r => { const analysis = readYamlFile(path.join(r, ".ai", "source-artifacts", "safe-structure-change-analysis.yaml")); if (analysis.artifact !== "safe_structure_change_analysis" || analysis.rollout_mode !== "observe" || analysis.enforcement_mode !== "none" || analysis.blocking_decision !== false) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Safe structure analysis did not emit observe-mode non-blocking result.")); } },
