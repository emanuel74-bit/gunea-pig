...
function writeEvidenceManifestFixture(fixtureRoot: string, entries: JsonMap[]): void {
  writeYamlFile(path.join(fixtureRoot, ".ai", "reports", "evidence-manifest.yaml"), {
    ...
    entries: entries.map((entry: JsonMap, index: number) => ({
      ...
      producer_route_id: entry.producer_route_id ?? String(entry.generated_by ?? "verify-scripts-fixture").replace(/-/g, "_"),
      route_produced: entry.route_produced ?? true,
...
function testCases(): TestCase[] {
  return [
    ...
    { name: "safe structure change analysis observes deletion risk", routeId: "analyze_safe_structure_change", script: "scripts/analyze-safe-structure-change.ts", category: "operation_correctness", args: ["--change-kind", "deletion", "--candidate-file", "conventions/scripts/validate-executor-routes.ts", "--enforcement-mode", "observe"], preRun: r => { const collected = runScript(r, "scripts/collect-source-artifacts.ts"); if (collected.exitCode !== 0) throw new Error(`Could not collect source artifacts fixture: ${collected.stderr}`); }, expectedExit: 0, expectedStatus: "pass", expectedWarningCodes: ["ROUTE_BACKED_OR_ROUTE_DECLARING_CANDIDATE", "INBOUND_DEPENDENCIES_PRESENT", "BEHAVIOR_PRESERVATION_EVIDENCE_MISSING", "CONSUMER_UPDATE_PLAN_MISSING"], expectedOutputs: [".ai/source-artifacts/safe-structure-change-analysis.yaml", ".ai/reports/safe-structure-change-report.yaml"], assert: r => { const analysis = readYamlFile(path.join(r, ".ai", "source-artifacts", "safe-structure-change-analysis.yaml")); if (analysis.artifact !== "safe_structure_change_analysis" || analysis.rollout_mode !== "observe" || analysis.enforcement_mode !== "observe" || analysis.blocking_decision !== false) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Safe structure analysis did not emit observe-mode non-blocking result.")); } },
    { name: "safe structure change analysis blocks unsafe deletion in controlled enforcement", routeId: "analyze_safe_structure_change", script: "scripts/analyze-safe-structure-change.ts", category: "invalid_state", args: ["--change-kind", "deletion", "--candidate-file", "conventions/scripts/validate-executor-routes.ts"], preRun: r => { const collected = runScript(r, "scripts/collect-source-artifacts.ts"); if (collected.exitCode !== 0) throw new Error(`Could not collect source artifacts fixture: ${collected.stderr}`); }, expectedExit: 1, expectedStatus: "fail", expectedErrorCodes: ["ROUTE_BACKED_OR_ROUTE_DECLARING_CANDIDATE", "INBOUND_DEPENDENCIES_PRESENT", "BEHAVIOR_PRESERVATION_EVIDENCE_MISSING", "CONSUMER_UPDATE_PLAN_MISSING"], expectedOutputs: [".ai/source-artifacts/safe-structure-change-analysis.yaml", ".ai/reports/safe-structure-change-report.yaml"], assert: r => { const analysis = readYamlFile(path.join(r, ".ai", "source-artifacts", "safe-structure-change-analysis.yaml")); if (analysis.rollout_mode !== "controlled_enforce" || analysis.enforcement_mode !== "controlled_enforce" || analysis.blocking_decision !== true) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Safe structure controlled enforcement did not block unsafe deletion.")); } },
    { name: "safe structure change analysis passes evidenced deletion in controlled enforcement", routeId: "analyze_safe_structure_change", script: "scripts/analyze-safe-structure-change.ts", category: "valid_input", args: ["--change-kind", "deletion", "--candidate-file", "conventions/scripts/validate-executor-routes.ts", "--behavior-evidence", "verified", "--consumer-update-plan", "updated", "--route-migration-evidence", "replacement-route", "--ownership-transfer-evidence", "ownership-transferred"], preRun: r => { const collected = runScript(r, "scripts/collect-source-artifacts.ts"); if (collected.exitCode !== 0) throw new Error(`Could not collect source artifacts fixture: ${collected.stderr}`); }, expectedExit: 0, expectedStatus: "pass", expectedOutputs: [".ai/source-artifacts/safe-structure-change-analysis.yaml", ".ai/reports/safe-structure-change-report.yaml"], assert: r => { const analysis = readYamlFile(path.join(r, ".ai", "source-artifacts", "safe-structure-change-analysis.yaml")); if (analysis.rollout_mode !== "controlled_enforce" || analysis.blocking_decision !== false || analysis.requested_change?.behavior_evidence_provided !== true || analysis.requested_change?.consumer_update_plan_provided !== true) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Evidenced safe structure deletion did not pass controlled enforcement.")); } },
    { name: "safe structure change analysis fails invalid enforcement mode", routeId: "analyze_safe_structure_change", script: "scripts/analyze-safe-structure-change.ts", category: "malformed_input", args: ["--enforcement-mode", "unsafe_magic"], preRun: r => { const collected = runScript(r, "scripts/collect-source-artifacts.ts"); if (collected.exitCode !== 0) throw new Error(`Could not collect source artifacts fixture: ${collected.stderr}`); }, expectedExit: 1, expectedStatus: "fail", expectedErrorCodes: ["UNKNOWN_SAFE_STRUCTURE_ENFORCEMENT_MODE"] },
    { name: "safe structure change analysis fails unknown change kind", routeId: "analyze_safe_structure_change", script: "scripts/analyze-safe-structure-change.ts", category: "malformed_input", args: ["--change-kind", "unsafe_magic"], preRun: r => { const collected = runScript(r, "scripts/collect-source-artifacts.ts"); if (collected.exitCode !== 0) throw new Error(`Could not collect source artifacts fixture: ${collected.stderr}`); }, expectedExit: 1, expectedStatus: "fail", expectedErrorCodes: ["UNKNOWN_STRUCTURE_CHANGE_KIND"] },
    { name: "safe structure change analysis fails missing manifest", routeId: "analyze_safe_structure_change", script: "scripts/analyze-safe-structure-change.ts", category: "missing_input", expectedExit: 1, expectedStatus: "fail", expectedErrorCodes: ["SOURCE_ARTIFACT_MANIFEST_MISSING"] },
    ...
    { name: "adapter topology compiles adapter declarations", routeId: "compile_adapter_topology", script: "scripts/compile-adapter-topology.ts", category: "operation_correctness", expectedExit: 0, expectedStatus: "pass", expectedOutputs: [".ai/topologies/adapter-topology.yaml", ".ai/reports/adapter-topology-report.yaml"], assert: r => { const topology = readYamlFile(path.join(r, ".ai", "topologies", "adapter-topology.yaml")); if (topology.artifact !== "adapter_topology" || topology.core_contract?.adapter_neutral !== true || topology.core_contract?.hardcoded_adapter_discovery_allowed !== false || !Array.isArray(topology.adapters) || topology.adapters.length < 2) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Adapter topology compiler did not produce adapter-neutral topology with discovered adapters.")); const routeIds = topology.adapters.map((entry: any) => entry.producer_route_id).filter(Boolean); if (!routeIds.includes("analyze_typescript_source") || !routeIds.includes("analyze_nestjs_source")) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Adapter topology compiler did not discover existing adapter producer routes.")); } },
    { name: "adapter topology fails without adapter declarations", routeId: "compile_adapter_topology", script: "scripts/compile-adapter-topology.ts", category: "missing_input", setup: r => { fs.rmSync(path.join(r, "adapters"), { recursive: true, force: true }); fs.mkdirSync(path.join(r, "adapters"), { recursive: true }); }, expectedExit: 1, expectedStatus: "fail", expectedErrorCodes: ["ADAPTER_DECLARATIONS_MISSING"] },
    { name: "architecture quality analysis observes adapter-neutral evidence", routeId: "analyze_architecture_quality", script: "scripts/analyze-architecture-quality.ts", category: "operation_correctness", preRun: r => { runScript(r, "scripts/collect-source-artifacts.ts"); runScript(r, "scripts/analyze-typescript-source.ts"); runScript(r, "scripts/analyze-nestjs-source.ts"); runScript(r, "scripts/compile-adapter-topology.ts"); runScript(r, "scripts/resolve-mission-mode-policy.ts", ["--mission-profile", "bugfix"]); }, expectedExit: 0, expectedStatus: "pass", expectedOutputs: [".ai/source-artifacts/architecture-quality-analysis.yaml", ".ai/reports/architecture-quality-report.yaml"], assert: r => { const analysis = readYamlFile(path.join(r, ".ai", "source-artifacts", "architecture-quality-analysis.yaml")); if (analysis.artifact !== "architecture_quality_analysis" || analysis.rollout_mode !== "controlled" || analysis.enforcement_mode !== "controlled_enforce" || analysis.blocking_decision !== false) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Architecture quality analysis did not emit controlled-enforce non-blocking result for advisory-only warnings.")); if (analysis.core_contract?.adapter_neutral !== true || analysis.core_contract?.requires_specific_language_adapter !== false || analysis.core_contract?.requires_specific_framework_adapter !== false) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Architecture quality core is not explicitly adapter-neutral.")); if (!Array.isArray(analysis.quality_dimensions) || analysis.quality_dimensions.length < 8 || analysis.evidence_inputs?.source_artifact_manifest !== true || analysis.evidence_inputs?.adapter_topology !== true || analysis.core_contract?.adapter_discovery_source !== "generated_adapter_topology" || analysis.core_contract?.hardcoded_adapter_discovery_allowed !== false || !Array.isArray(analysis.evidence_inputs?.language_analysis_outputs) || !Array.isArray(analysis.evidence_inputs?.framework_analysis_outputs)) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Architecture quality analysis did not expose expected adapter-neutral evidence inputs.")); const result = readYamlFile(path.join(r, ".ai", "validation", "analyze-architecture-quality.result.yaml")); if (result.summary?.validation_result?.route_id !== "analyze_architecture_quality" || result.summary?.validation_result?.blocking !== false || result.summary?.adapter_neutral_core !== true) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Architecture quality analysis did not emit adapter-neutral universal validation_result metadata.")); } },
    { name: "architecture quality analysis remains non-blocking without optional adapters", routeId: "analyze_architecture_quality", script: "scripts/analyze-architecture-quality.ts", category: "valid_input", preRun: r => { runScript(r, "scripts/collect-source-artifacts.ts"); runScript(r, "scripts/compile-adapter-topology.ts"); }, expectedExit: 0, expectedStatus: "pass", expectedWarningCodes: ["LANGUAGE_ADAPTER_EVIDENCE_MISSING", "FRAMEWORK_ADAPTER_EVIDENCE_MISSING", "MISSION_MODE_POLICY_MISSING"], expectedOutputs: [".ai/source-artifacts/architecture-quality-analysis.yaml", ".ai/reports/architecture-quality-report.yaml"], assert: r => { const analysis = readYamlFile(path.join(r, ".ai", "source-artifacts", "architecture-quality-analysis.yaml")); if (!Array.isArray(analysis.evidence_inputs?.language_analysis_outputs) || analysis.evidence_inputs.language_analysis_outputs.length !== 0 || !Array.isArray(analysis.evidence_inputs?.framework_analysis_outputs) || analysis.evidence_inputs.framework_analysis_outputs.length !== 0 || analysis.blocking_decision !== false || analysis.core_contract?.adapter_neutral !== true) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Architecture quality missing-adapter path must remain adapter-neutral and non-blocking for advisory-only warnings.")); } },
    { name: "architecture quality controlled enforcement blocks safe-structure blocking signal", routeId: "analyze_architecture_quality", script: "scripts/analyze-architecture-quality.ts", category: "operation_correctness", preRun: r => { runScript(r, "scripts/collect-source-artifacts.ts"); runScript(r, "scripts/compile-adapter-topology.ts"); writeYamlFile(path.join(r, ".ai", "source-artifacts", "safe-structure-change-analysis.yaml"), { artifact: "safe_structure_change_analysis", generated_by: "analyze-safe-structure-change", blocking_decision: true }); }, expectedExit: 1, expectedStatus: "fail", expectedErrorCodes: ["ARCH_SAFE_STRUCTURE_BLOCKING_SIGNAL"] },
    { name: "architecture quality observe override preserves safe-structure warning", routeId: "analyze_architecture_quality", script: "scripts/analyze-architecture-quality.ts", category: "operation_correctness", args: ["--enforcement-mode", "observe"], preRun: r => { runScript(r, "scripts/collect-source-artifacts.ts"); runScript(r, "scripts/compile-adapter-topology.ts"); writeYamlFile(path.join(r, ".ai", "source-artifacts", "safe-structure-change-analysis.yaml"), { artifact: "safe_structure_change_analysis", generated_by: "analyze-safe-structure-change", blocking_decision: true }); }, expectedExit: 0, expectedStatus: "pass", expectedWarningCodes: ["ARCH_SAFE_STRUCTURE_BLOCKING_SIGNAL"], assert: r => { const analysis = readYamlFile(path.join(r, ".ai", "source-artifacts", "architecture-quality-analysis.yaml")); if (analysis.enforcement_mode !== "observe" || analysis.blocking_decision !== false) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Architecture quality observe override did not preserve warning-only behavior.")); } },
    { name: "architecture quality fails invalid enforcement mode", routeId: "analyze_architecture_quality", script: "scripts/analyze-architecture-quality.ts", category: "malformed_input", args: ["--enforcement-mode", "strict"], expectedExit: 1, expectedStatus: "fail", expectedErrorCodes: ["ARCHITECTURE_QUALITY_ENFORCEMENT_MODE_INVALID"] },
    { name: "architecture quality analysis fails missing adapter topology", routeId: "analyze_architecture_quality", script: "scripts/analyze-architecture-quality.ts", category: "missing_input", preRun: r => { runScript(r, "scripts/collect-source-artifacts.ts"); }, expectedExit: 1, expectedStatus: "fail", expectedErrorCodes: ["ADAPTER_TOPOLOGY_MISSING"] },
    { name: "architecture quality analysis fails missing manifest", routeId: "analyze_architecture_quality", script: "scripts/analyze-architecture-quality.ts", category: "missing_input", expectedExit: 1, expectedStatus: "fail", expectedErrorCodes: ["SOURCE_ARTIFACT_MANIFEST_MISSING"] },
    { name: "global normalization analysis observes topology-backed signals", routeId: "analyze_global_normalization", script: "scripts/analyze-global-normalization.ts", category: "operation_correctness", preRun: r => { runScript(r, "scripts/collect-source-artifacts.ts"); runScript(r, "scripts/analyze-typescript-source.ts"); runScript(r, "scripts/analyze-nestjs-source.ts"); runScript(r, "scripts/compile-adapter-topology.ts"); runScript(r, "scripts/resolve-mission-mode-policy.ts", ["--mission-profile", "bugfix"]); runScript(r, "scripts/analyze-architecture-quality.ts", ["--enforcement-mode", "observe"]); }, expectedExit: 0, expectedStatus: "pass", expectedOutputs: [".ai/source-artifacts/global-normalization-analysis.yaml", ".ai/reports/global-normalization-report.yaml"], assert: r => { const analysis = readYamlFile(path.join(r, ".ai", "source-artifacts", "global-normalization-analysis.yaml")); const dimensions = Array.isArray(analysis.normalization_dimensions) ? analysis.normalization_dimensions.map((entry: any) => entry?.dimension) : []; if (analysis.artifact !== "global_normalization_analysis" || analysis.rollout_mode !== "observe" || analysis.enforcement_mode !== "observe" || analysis.blocking_decision !== false || analysis.mutation_allowed !== false || analysis.adapter_neutral !== true || analysis.policy_abstraction !== "universal_normalization_dimensions" || analysis.concrete_signals_are_policy !== false || analysis.project_specific_rules_allowed !== false || !dimensions.includes("conceptual_overlap") || !dimensions.includes("responsibility_granularity") || !dimensions.includes("shared_mechanism_consistency") || !analysis.detector_signals) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Global normalization analysis did not emit abstract adapter-neutral normalization output.")); } },
    { name: "global normalization analysis fails missing adapter topology", routeId: "analyze_global_normalization", script: "scripts/analyze-global-normalization.ts", category: "missing_input", preRun: r => { runScript(r, "scripts/collect-source-artifacts.ts"); }, expectedExit: 1, expectedStatus: "fail", expectedErrorCodes: ["ADAPTER_TOPOLOGY_MISSING"] },
    { name: "global normalization analysis fails invalid enforcement mode", routeId: "analyze_global_normalization", script: "scripts/analyze-global-normalization.ts", category: "malformed_input", args: ["--enforcement-mode", "strict"], preRun: r => { runScript(r, "scripts/collect-source-artifacts.ts"); runScript(r, "scripts/compile-adapter-topology.ts"); }, expectedExit: 1, expectedStatus: "fail", expectedErrorCodes: ["GLOBAL_NORMALIZATION_ENFORCEMENT_MODE_INVALID"] },
    ...
    { name: "evidence collection indexes safe structure observe analysis", routeId: "collect_evidence", script: "scripts/collect-evidence.ts", category: "operation_correctness", preRun: r => { const collected = runScript(r, "scripts/collect-source-artifacts.ts"); if (collected.exitCode !== 0) throw new Error(`Could not collect source artifacts fixture: ${collected.stderr}`); runScript(r, "scripts/analyze-safe-structure-change.ts", ["--change-kind", "deletion", "--candidate-file", "conventions/scripts/validate-executor-routes.ts", "--enforcement-mode", "observe"]); }, expectedExit: 0, expectedStatus: "pass", expectedOutputs: [".ai/reports/evidence-manifest.yaml", ".ai/reports/evidence-index.yaml", ".ai/reports/evidence-collection-report.yaml"], assert: r => { const index = readYamlFile(path.join(r, ".ai", "reports", "evidence-index.yaml")); const entry = Array.isArray(index.entries) ? index.entries.find((candidate: any) => candidate.path === ".ai/source-artifacts/safe-structure-change-analysis.yaml") : undefined; if (!entry || entry.source_type !== "source_artifact" || entry.blocking !== false || entry.safe_structure_analysis?.change_kind !== "deletion" || entry.safe_structure_analysis?.blocking_decision !== false || !Array.isArray(entry.safe_structure_analysis?.warning_codes) || !entry.safe_structure_analysis.warning_codes.includes("BEHAVIOR_PRESERVATION_EVIDENCE_MISSING")) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Evidence index did not expose observe-mode safe structure analysis fields without blocking.")); } },
    { name: "evidence collection indexes adapter-neutral architecture quality analysis", routeId: "collect_evidence", script: "scripts/collect-evidence.ts", category: "operation_correctness", preRun: r => { runScript(r, "scripts/collect-source-artifacts.ts"); runScript(r, "scripts/analyze-typescript-source.ts"); runScript(r, "scripts/analyze-nestjs-source.ts"); runScript(r, "scripts/compile-adapter-topology.ts"); runScript(r, "scripts/resolve-mission-mode-policy.ts", ["--mission-profile", "bugfix"]); runScript(r, "scripts/analyze-architecture-quality.ts"); }, expectedExit: 0, expectedStatus: "pass", expectedOutputs: [".ai/reports/evidence-manifest.yaml", ".ai/reports/evidence-index.yaml", ".ai/reports/evidence-collection-report.yaml"], assert: r => { const index = readYamlFile(path.join(r, ".ai", "reports", "evidence-index.yaml")); const entry = Array.isArray(index.entries) ? index.entries.find((candidate: any) => candidate.path === ".ai/source-artifacts/architecture-quality-analysis.yaml") : undefined; if (!entry || entry.source_type !== "source_artifact" || entry.blocking !== false || entry.architecture_quality_analysis?.adapter_neutral !== true || entry.architecture_quality_analysis?.adapter_discovery_source !== "generated_adapter_topology" || entry.architecture_quality_analysis?.quality_dimension_count < 8 || entry.architecture_quality_analysis?.blocking_decision !== false) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Evidence index did not expose topology-backed adapter-neutral architecture quality analysis fields without blocking.")); } },
    ...
    { name: "report evidence validation rejects claim ref outside manifest", routeId: "validate_report_evidence", script: "scripts/validate-report-evidence.ts", category: "invalid_state", setup: r => { writeYamlFile(path.join(r, ".ai", "validation", "claimed.yaml"), { artifact: "validation_result", generated_by: "validate-executor-routes", status: "pass" }); writeYamlFile(path.join(r, ".ai", "validation", "not-in-manifest.yaml"), { artifact: "validation_result", generated_by: "validate-executor-routes", status: "pass" }); writeEvidenceManifestFixture(r, [{ evidence_id: "claimed", path: ".ai/validation/claimed.yaml", source_type: "validation", generated_by: "validate-executor-routes", producer_route_id: "validate_executor_routes", route_produced: true, status: "pass" }]); writeYamlFile(path.join(r, ".ai", "reports", "final-mission-report.yaml"), { artifact: "final_mission_report", generated_by: "generate-final-mission-report", status: "complete", evidence_manifest: ".ai/reports/evidence-manifest.yaml", evidence_refs: [".ai/validation/claimed.yaml"], report_claim_schema: { schema_version: "1.0" }, report_claims: [{ claim_schema_version: "1.0", claim_id: "bad_claim", claim_type: "validation", claim_status: "supported", support_mode: "manifest_backed", evidence_refs: [".ai/validation/not-in-manifest.yaml"] }] }); }, expectedExit: 1, expectedStatus: "fail", expectedErrorCodes: ["REPORT_CLAIM_EVIDENCE_REF_NOT_IN_MANIFEST"] },
    { name: "report evidence validation rejects unsupported final claim", routeId: "validate_report_evidence", script: "scripts/validate-report-evidence.ts", category: "invalid_state", setup: r => { writeYamlFile(path.join(r, ".ai", "validation", "claimed.yaml"), { artifact: "validation_result", generated_by: "validate-executor-routes", status: "pass" }); writeEvidenceManifestFixture(r, [{ evidence_id: "claimed", path: ".ai/validation/claimed.yaml", source_type: "validation", generated_by: "validate-executor-routes", producer_route_id: "validate_executor_routes", route_produced: true, status: "pass" }]); writeYamlFile(path.join(r, ".ai", "reports", "final-mission-report.yaml"), { artifact: "final_mission_report", generated_by: "generate-final-mission-report", status: "complete", evidence_manifest: ".ai/reports/evidence-manifest.yaml", evidence_refs: [".ai/validation/claimed.yaml"], report_claim_schema: { schema_version: "1.0" }, report_claims: [{ claim_schema_version: "1.0", claim_id: "unsupported_claim", claim_type: "validation", claim_status: "unsupported", support_mode: "manifest_backed", evidence_refs: [".ai/validation/claimed.yaml"] }] }); }, expectedExit: 1, expectedStatus: "fail", expectedErrorCodes: ["REPORT_CLAIM_UNSUPPORTED"] },
    { name: "report evidence validation rejects unbacked narrative section", routeId: "validate_report_evidence", script: "scripts/validate-report-evidence.ts", category: "invalid_state", setup: r => { writeYamlFile(path.join(r, ".ai", "validation", "claimed.yaml"), { artifact: "validation_result", generated_by: "validate-executor-routes", status: "pass" }); writeEvidenceManifestFixture(r, [{ evidence_id: "claimed", path: ".ai/validation/claimed.yaml", source_type: "validation", generated_by: "validate-executor-routes", producer_route_id: "validate_executor_routes", route_produced: true, status: "pass" }]); writeYamlFile(path.join(r, ".ai", "reports", "final-mission-report.yaml"), { artifact: "final_mission_report", generated_by: "generate-final-mission-report", status: "complete", summary_source: "typed_report_claims", evidence_manifest: ".ai/reports/evidence-manifest.yaml", evidence_refs: [".ai/validation/claimed.yaml"], report_claim_schema: { schema_version: "1.0" }, report_claims: [{ claim_schema_version: "1.0", claim_id: "validation_summary_backed", claim_type: "validation", claim_status: "supported", support_mode: "manifest_backed", evidence_refs: [".ai/validation/claimed.yaml"] }], narrative_summary: { narrative_schema_version: "1.0", generated_from_report_claims: true, freeform_narrative_allowed: false, sections: [{ section_schema_version: "1.0", section_id: "bad_narrative", title: "Bad narrative", generated_from: "report_claim", source_claim_id: "missing_claim", claim_status: "supported", evidence_refs: [".ai/validation/claimed.yaml"], summary_text: "Unbacked narrative." }] } }); }, expectedExit: 1, expectedStatus: "fail", expectedErrorCodes: ["FINAL_REPORT_NARRATIVE_SOURCE_CLAIM_MISSING"] },
    ...
    { name: "final mission report generation writes evidence backed report", routeId: "generate_final_mission_report", script: "scripts/generate-final-mission-report.ts", category: "valid_input", preRun: r => { runScript(r, "scripts/validate-executor-routes.ts"); runScript(r, "scripts/collect-evidence.ts"); }, expectedExit: 0, expectedStatus: "pass", expectedOutputs: [".ai/reports/final-mission-report.yaml", ".ai/reports/mission-evidence-summary.yaml"], assert: r => { const report = readYamlFile(path.join(r, ".ai", "reports", "final-mission-report.yaml")); if (!report.report_claim_schema || report.report_claim_schema.schema_version !== "1.0") issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Final mission report did not emit report claim schema."));
      if (!Array.isArray(report.report_claims) || report.report_claims.length === 0 || !report.report_claims.every((claim: JsonMap) => claim.claim_schema_version === "1.0" && (claim.claim_status === "not_applicable" ? typeof claim.not_applicable_reason === "string" : Array.isArray(claim.evidence_refs) && claim.evidence_refs.length > 0))) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Final mission report did not emit schema-backed manifest-backed report claims."));
      if (report.summary_source !== "typed_report_claims" || report.narrative_summary?.generated_from_report_claims !== true || report.narrative_summary?.freeform_narrative_allowed !== false || !Array.isArray(report.narrative_summary?.sections) || !report.narrative_summary.sections.every((section: JsonMap) => section.generated_from === "report_claim" && typeof section.source_claim_id === "string")) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Final mission report did not emit typed claim-derived narrative summary.")); } },
    ...
    { name: "final mission report summarizes observe safe structure without blocking", routeId: "generate_final_mission_report", script: "scripts/generate-final-mission-report.ts", category: "operation_correctness", setup: r => writeEvidenceManifestFixture(r, [{ evidence_id: "safe_structure_analysis", path: ".ai/source-artifacts/safe-structure-change-analysis.yaml", source_type: "source_artifact", generated_by: "analyze-safe-structure-change", status: "pass", blocking: false, safe_structure_analysis: { change_kind: "deletion", candidate_file: "conventions/scripts/validate-executor-routes.ts", target_file: null, inbound_dependency_count: 2, outbound_dependency_count: 1, warning_codes: ["BEHAVIOR_PRESERVATION_EVIDENCE_MISSING"], blocking_decision: false } }]), expectedExit: 0, expectedStatus: "pass", expectedOutputs: [".ai/reports/final-mission-report.yaml", ".ai/reports/mission-evidence-summary.yaml"], assert: r => { const report = readYamlFile(path.join(r, ".ai", "reports", "final-mission-report.yaml")); if (report.status !== "complete" || report.safe_structure_summary?.safe_structure_count !== 1 || report.safe_structure_summary?.observe_mode_only !== true || report.safe_structure_summary?.total_inbound_dependency_count !== 2 || report.safe_structure_summary?.analyses?.[0]?.blocking_decision !== false) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Final mission report did not summarize observe-mode safe structure analysis without blocking.")); const claim = Array.isArray(report.report_claims) ? report.report_claims.find((entry: JsonMap) => entry.claim_id === "safe_structure_summary_backed") : undefined; if (!claim || claim.claim_status !== "supported" || !Array.isArray(claim.evidence_refs) || !claim.evidence_refs.includes(".ai/source-artifacts/safe-structure-change-analysis.yaml")) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Final mission report did not emit manifest-backed safe structure claim.")); } },
    { name: "final mission report summarizes adapter-neutral architecture quality without blocking", routeId: "generate_final_mission_report", script: "scripts/generate-final-mission-report.ts", category: "operation_correctness", setup: r => writeEvidenceManifestFixture(r, [{ evidence_id: "architecture_quality_analysis", path: ".ai/source-artifacts/architecture-quality-analysis.yaml", source_type: "source_artifact", generated_by: "analyze-architecture-quality", status: "pass", blocking: false, architecture_quality_analysis: { rollout_mode: "observe", enforcement_mode: "observe", blocking_decision: false, adapter_neutral: true, adapter_discovery_source: "generated_adapter_topology", language_adapter_count: 1, framework_adapter_count: 1, quality_dimension_count: 8, warning_codes: ["ARCH_LONG_FILE_RISK"] } }]), expectedExit: 0, expectedStatus: "pass", expectedOutputs: [".ai/reports/final-mission-report.yaml", ".ai/reports/mission-evidence-summary.yaml"], assert: r => { const report = readYamlFile(path.join(r, ".ai", "reports", "final-mission-report.yaml")); if (report.status !== "complete" || report.architecture_quality_summary?.architecture_quality_count !== 1 || report.architecture_quality_summary?.observe_mode_only !== true || report.architecture_quality_summary?.adapter_neutral_core !== true || report.architecture_quality_summary?.total_quality_dimension_count !== 8 || report.architecture_quality_summary?.analyses?.[0]?.adapter_discovery_source !== "generated_adapter_topology") issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Final mission report did not summarize adapter-neutral architecture quality analysis without blocking.")); const claim = Array.isArray(report.report_claims) ? report.report_claims.find((entry: JsonMap) => entry.claim_id === "architecture_quality_summary_backed") : undefined; if (!claim || claim.claim_status !== "supported" || !Array.isArray(claim.evidence_refs) || !claim.evidence_refs.includes(".ai/source-artifacts/architecture-quality-analysis.yaml")) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Final mission report did not emit manifest-backed architecture quality claim.")); } },
    { name: "evidence collection indexes abstract global normalization analysis", routeId: "collect_evidence", script: "scripts/collect-evidence.ts", category: "operation_correctness", preRun: r => { runScript(r, "scripts/collect-source-artifacts.ts"); runScript(r, "scripts/compile-adapter-topology.ts"); runScript(r, "scripts/analyze-global-normalization.ts"); }, expectedExit: 0, expectedStatus: "pass", expectedOutputs: [".ai/reports/evidence-manifest.yaml", ".ai/reports/evidence-index.yaml", ".ai/reports/evidence-collection-report.yaml"], assert: r => { const index = readYamlFile(path.join(r, ".ai", "reports", "evidence-index.yaml")); const entry = Array.isArray(index.entries) ? index.entries.find((candidate: any) => candidate.path === ".ai/source-artifacts/global-normalization-analysis.yaml") : undefined; if (!entry || entry.source_type !== "source_artifact" || entry.blocking !== false || entry.global_normalization_analysis?.adapter_neutral !== true || entry.global_normalization_analysis?.policy_abstraction !== "universal_normalization_dimensions" || entry.global_normalization_analysis?.concrete_signals_are_policy !== false || entry.global_normalization_analysis?.project_specific_rules_allowed !== false || entry.global_normalization_analysis?.mutation_allowed !== false || entry.global_normalization_analysis?.normalization_dimension_count < 7) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Evidence index did not expose abstract global normalization analysis fields without blocking.")); } },
    { name: "final mission report summarizes abstract global normalization without blocking", routeId: "generate_final_mission_report", script: "scripts/generate-final-mission-report.ts", category: "operation_correctness", setup: r => writeEvidenceManifestFixture(r, [{ evidence_id: "global_normalization_analysis", path: ".ai/source-artifacts/global-normalization-analysis.yaml", source_type: "source_artifact", generated_by: "analyze-global-normalization", status: "pass", blocking: false, global_normalization_analysis: { rollout_mode: "observe", enforcement_mode: "observe", blocking_decision: false, adapter_neutral: true, policy_abstraction: "universal_normalization_dimensions", concrete_signals_are_policy: false, project_specific_rules_allowed: false, mutation_allowed: false, automatic_rewrites_allowed: false, normalization_dimension_count: 7, adapter_count: 2, detector_signal_keys: ["repeated_base_name_groups"], finding_keys: ["conceptual_overlap"], warning_codes: ["GLOBAL_NORMALIZATION_CONCEPTUAL_OVERLAP_SIGNAL"] } }]), expectedExit: 0, expectedStatus: "pass", expectedOutputs: [".ai/reports/final-mission-report.yaml", ".ai/reports/mission-evidence-summary.yaml"], assert: r => { const report = readYamlFile(path.join(r, ".ai", "reports", "final-mission-report.yaml")); if (report.status !== "complete" || report.global_normalization_summary?.global_normalization_count !== 1 || report.global_normalization_summary?.observe_mode_only !== true || report.global_normalization_summary?.adapter_neutral_core !== true || report.global_normalization_summary?.abstract_policy_only !== true || report.global_normalization_summary?.mutation_allowed !== false || report.global_normalization_summary?.total_normalization_dimension_count !== 7 || report.global_normalization_summary?.analyses?.[0]?.policy_abstraction !== "universal_normalization_dimensions") issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Final mission report did not summarize abstract global normalization analysis without blocking.")); const claim = Array.isArray(report.report_claims) ? report.report_claims.find((entry: JsonMap) => entry.claim_id === "global_normalization_summary_backed") : undefined; if (!claim || claim.claim_status !== "supported" || !Array.isArray(claim.evidence_refs) || !claim.evidence_refs.includes(".ai/source-artifacts/global-normalization-analysis.yaml")) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Final mission report did not emit manifest-backed global normalization claim.")); } },
...
const liveTestNames = new Set([
  ...
  "global normalization analysis observes topology-backed signals",
  "safe structure change analysis observes deletion risk",
