...
function testCases(): TestCase[] {
  return [
    ...
    { name: "revision loop analysis emits baseline observe warning", routeId: "analyze_revision_loop", script: "scripts/analyze-revision-loop.ts", category: "valid_input", expectedExit: 0, expectedStatus: "pass", expectedWarningCodes: ["REVISION_FAILURE_FINGERPRINT_MISSING"], expectedOutputs: [".ai/revisions/revision-loop-analysis.yaml", ".ai/reports/revision-loop-analysis-report.yaml"], assert: r => { const analysis = readYamlFile(path.join(r, ".ai", "revisions", "revision-loop-analysis.yaml")); if (analysis.artifact !== "revision_loop_analysis" || analysis.rollout_mode !== "observe" || analysis.blocking_decision !== false) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Revision loop analysis did not emit observe-mode baseline analysis.")); } },
    { name: "revision loop analysis warns repeated fingerprint", routeId: "analyze_revision_loop", script: "scripts/analyze-revision-loop.ts", category: "operation_correctness", args: ["--failure-fingerprint", "repeat123", "--observe-retry-budget", "2"], setup: r => { writeYamlFile(path.join(r, ".ai", "revisions", "repeat-a.yaml"), { artifact: "revision_task", generated_by: "create-revision-task", revision_id: "repeat-a", status: "created", failure: "same", failure_fingerprint: "repeat123", required_action: "resolve_blocking_failure_and_revalidate", validation_required_routes: ["validate_revision_task", "validate_changed_files"] }); writeYamlFile(path.join(r, ".ai", "revisions", "repeat-b.yaml"), { artifact: "revision_task", generated_by: "create-revision-task", revision_id: "repeat-b", status: "created", failure: "same", failure_fingerprint: "repeat123", required_action: "resolve_blocking_failure_and_revalidate", validation_required_routes: ["validate_revision_task", "validate_changed_files"] }); }, expectedExit: 0, expectedStatus: "pass", expectedWarningCodes: ["REVISION_FAILURE_FINGERPRINT_REPEATED", "REVISION_RETRY_BUDGET_OBSERVED"], expectedOutputs: [".ai/revisions/revision-loop-analysis.yaml", ".ai/reports/revision-loop-analysis-report.yaml"], assert: r => { const analysis = readYamlFile(path.join(r, ".ai", "revisions", "revision-loop-analysis.yaml")); if (analysis.active_fingerprint_count !== 2 || analysis.no_progress_suspected !== true || analysis.blocking_decision !== false) issues.push(issue("error", "SCRIPT_ASSERTION_FAILED", "Revision loop analysis did not observe repeated fingerprint without blocking.")); } },
...
const liveTestNames = new Set([
  ...
  "revision loop analysis emits baseline observe warning",
