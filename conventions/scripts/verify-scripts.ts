...
function testCases(): TestCase[] {
  return [
    ...
    { name: "claude native integration validation passes project files", routeId: "validate_claude_native_integration", script: "scripts/validate-claude-native-integration.ts", category: "valid_input", setup: r => {
      ...
      const skills = ["start-mission", "run-alpha-cycle", "activate-agent", "run-phase", "validate-phase", "run-gate", "prepare-handoff", "transition-phase", "revision-loop", "generate-artifact", "collect-report-evidence", "complete-mission"];
      const commands = ["start-mission", "run-alpha-cycle", "activate-agent", "run-phase", "validate-phase", "transition-phase", "revision-loop", "complete-mission"];
      const coreRoutes = ["generate_claude_execution_plan", "validate_claude_execution_plan", "generate_phase_bundle", "validate_phase_bundle", "validate_changed_files", "validate_phase_output", "validate_semantic_completeness", "run_gate_check", "validate_gate_result", "prepare_agent_handoff", "validate_handoff", "create_revision_task", "validate_revision_task", "collect_evidence", "validate_report_evidence", "generate_final_mission_report"];
      fs.writeFileSync(path.join(projectRoot, "CLAUDE.md"), coreRoutes.map(route => `npm --prefix conventions/scripts run executor -- --route ${route}`).join("\n"), "utf8");
    ...
    }, expectedExit: 0, expectedStatus: "pass", expectedOutputs: [".ai/validation/validate-claude-native-integration.result.yaml"] },
  ];
}
