...
if (!fs.existsSync(topologyPath)) {
  ...
  for (const [artifactId, artifact] of Object.entries<any>(artifacts)) {
    const artifactPath = String(artifact.path ?? "");
    if (!artifactId) issues.push(issue("error", "RUNTIME_ARTIFACT_ID_MISSING", "Runtime artifact entry is missing artifact_id"));
