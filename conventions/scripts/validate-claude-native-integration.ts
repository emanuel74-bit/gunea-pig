...
const agentDefinitions = (agentTaxonomy?.sections?.agent_definitions ?? {}) as Record<string, unknown>;
...
function projectFilePath(relPath: string): string {
...
const nativeAgents = (activationMap?.sections?.native_agents ?? {}) as Record<string, { claude_agent_file?: string }>;
...
const lanes = (engagement?.sections?.workflow_lanes ?? {}) as Record<string, { subsystems?: string[]; routes?: string[]; agents?: string[] }>;
const mappedSubsystems = new Set<string>();
const mappedAgents = new Set<string>();
for (const [laneId, lane] of Object.entries(lanes)) {
  for (const subsystem of lane.subsystems ?? []) mappedSubsystems.add(subsystem);
