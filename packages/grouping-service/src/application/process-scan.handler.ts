...
export class ProcessScanHandler {
  ...
  async handle(event: ScanCompletedEvent): Promise<void> {
    ...
    const threshold = new SimilarityThreshold(this.config.thresholdPolicy);
  ...
  private async mergeComponent(videoIds: string[]): Promise<void> {
    ...
    const existingGroupIds = [...new Set(memberships.map((m) => m.similarityGroupId!).filter(Boolean))];
