...
description: Failure analysis agent that classifies failures and decides repair/rerun/revision/block path before any repair.
allowed-tools:
  - Read
  - Grep
  - Glob
...
Convention agent id: `failure_analysis_agent`  
...
Activate this subagent when the validated phase bundle names owner agent `failure_analysis_agent` or when the active phase is one of:
...
- assign owner phase
- select next action
- request revision when validation failure or blocker conditions are present
